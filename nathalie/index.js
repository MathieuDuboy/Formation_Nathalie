...
// Partie concernant le geocodage
// demander une clé API Google MAp pour un projet que vous allez créer.
// taper sur Google : signin Google map API KEY
...
var actions = {
  ...
  // action qui va aller chercher la météo de la ville X
  afficher_meteo_lat_lng( sessionId, context, entities, lat, lng ) {
    ...
    requestify.get("http://api.openweathermap.org/data/2.5/weather?APPID="+api_key_weather+"&lat="+lat+"&lon="+lng, {} ).then( function( response )  {
        ...
        actions.envoyer_message_text( sessionId, context, entities, "Il fait "+tempC+"°Celsius").then(function() {
          actions.envoyer_message_quickreplies(sessionId, context, entities, "Que souhaitez-vous faire maintenant ?", quick);
          // ici il faut stocker dans "villes" le nom de la ville recherchée
        })
      });
  },
  rechercher_dans_firebase_mes_recherches(sessionId, context, entities) {
    //effectuer une requete fireabse sur l'utilsiateur et afficher les villes qu'il a deja visité
      // si le tableau est vide, on va afficher " vous n'avez encore effectué aucune recherche"
      // sinon on va afficher dans un caroussel la ville1, la ville2, etc ...
      ...
      entry.messaging.forEach( event => {
        if ( event.message && !event.message.is_echo ) {
          ...
          if (event.message.attachments != null  && typeof event.message.attachments[0] != 'undefined') {
              // je viens de recevoir la latitude et la longitude d'un utilsiateur
              if(attachments[0].payload.coordinates && attachments[0].payload.coordinates.lat) {
                var latitude = attachments[0].payload.coordinates.lat;
                var longitude = attachments[0].payload.coordinates.long;
                actions.afficher_meteo_lat_lng( sessionId, context, entities, latitude, longitude );
              }
              else if(event.message.attachments[0].payload.url) {
                console.log(JSON.stringify(event.message.attachments[0].payload.url));
                var url_photo_sur_serveur_messenger = event.message.attachments[0].payload.url;
                // stocker sur firebase
                // stocker ailleurs
                // actions de remerciements d'avoir envoyé l'image
                actions.envoyer_message_text( sessionId, context, entities, "Merci de m'avoir envoyé la photo." );
                // etc
              }
					}
